import flet as ft
# flet_audio kütüphanesini kaldırdık, 0.25.2 sürümünde ses dahili gelir.
...
class MusicApp:
    ...
    def build_ui(self):
        ...
        self.shuffle_btn = ft.IconButton(icon="shuffle", icon_size=24, icon_color="white24", on_click=self.toggle_shuffle)
        self.repeat_btn = ft.IconButton(icon="repeat", icon_size=24, icon_color="white24", on_click=self.toggle_repeat)
                ...
                ft.Container(
                    ...
                    content=ft.Row([
                        ...
                        ft.Container(
                            ...
                            gradient=ft.LinearGradient(colors=["orange", "deepOrange"], begin=ft.alignment.center_left, end=ft.alignment.center_right),
                            border_radius=10,
                            padding=0,
    ...
    def sure_guncelle(self, e):
        ...
        try:
            pos = int(e.data)
            self.sure_slider.value = pos
            self.gecen_sure_txt.value = self.sure_formatla(pos)
            self.page.update()
        except: pass
        ...
        def background_load():
            try:
                ...
                def update_ui_safe():
                    ...
                    else: self.sure_slider.disabled = True 
    ...
    def toggle_play_pause(self, e):
        if self.audio_player and self.audio_player.src:
            if self.caliniyor_mu:
                ...
                self.caliniyor_mu = False
                self.resim_konteyner.content.scale = 0.95
    ...
    def indirme_baslat(self, e):
        if not self.indir_butonu.data: return
    ...
    def favori_islem(self, e):
        if not self.favori_butonu.data: return
    ...
    def videoyu_ac(self, e):
        if self.video_butonu.data: self.page.launch_url(f"https://www.youtube.com/watch?v={self.video_butonu.data}")
